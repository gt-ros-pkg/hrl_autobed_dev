...
class PoseTrainer():
    ...
    def __init__(self, pose_ID, training_database_file):
        ...
        and the pickle file stuff. Will read the pose_list pickle file to get the 
        joint positions corresponding to the pose_ID'''
        ...
        self.current_pressure_map = np.zeros(self.pressure_mat.get_mat_size_in_taxels()) 
    ...
    def begin_training(self):
        '''Calls the get_pressure_map() method given by the MapGenerator class 
        until the pressure map is rich enough to be stored in the database. 
        Once this is done, the map along with the list of joints is safely stored 
        in the training database pickle file'''

        num_pressure_points = 0 #Variable representing the number of pressure points recorded
        while num_pressure_points<290 and (not rospy.is_shutdown()):
            pressure_map_matrix = self.pressure_mat.get_pressure_map() #Get the current pressure mat readings
            '''Note: Since, we are using a Gazebo based pressure mat simulation
            for our training, we don't get a very rich pressure map within one reading. 
            Thus we will logically OR our previous pressure map reading with the 
            current one. Since we are not moving in the bed, this is fine.'''
        ...
        '''We have obtained enough pressure points to be stored. 
        Now lets interpolate the same and visualize the interpolated map'''
        #self.pressure_mat.interpolate_pressure_map(self.current_pressure_map)
        #self.pressure_mat.visualize_pressure_map(self.current_pressure_map)
        '''We have obtained a pressure map that is rich enough. 
        Lets upload that to the database now'''
        #assign the current pressure map as a value to the pose_ID key.
        self.training_database[self.pose_ID]['pressure_map'] = self.current_pressure_map 
